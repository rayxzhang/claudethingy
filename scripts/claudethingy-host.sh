...
RUN_DIR="$(carthingy_run_dir "$REPO_ROOT")"
PID_FILE="$RUN_DIR/host.pid"
LOG_FILE="$REPO_ROOT/claudethingy.log"
...
start_host() {
  local serial="${1:-}"
  if [[ ! -f "$PID_FILE" && -f "$REPO_ROOT/.carthingy/host.pid" ]]; then
    stop_host
  fi
  ...
  if [[ ! -x "$NODE_BIN" ]]; then
    carthingy_log "node not found at $NODE_BIN. Set CARTHINGY_NODE in claudethingy.conf"
    return 1
  fi
...
stop_host() {
  local dir pidfile
  for dir in "$RUN_DIR" "$REPO_ROOT/.carthingy"; do
    for pidfile in "$dir/rotary.pid" "$dir/reverse.pid" "$dir/host.pid"; do
      if [[ -f "$pidfile" ]]; then
